...
2. **針對每個領域，各想出 8 個可以關注或努力的方向**。例如「嗜好」這塊，底下可能是登山、閱讀、攝影。八個領域、每個八個方向，合起來排成 9×9 的格狀圖（中央格留給標題）。
...
- **不是即插即用的產品**，是一份參考實作。frontmatter 欄位是中文，使用前要自己調整。
...
荒廢只看「多久沒有新記錄」，更動方向的名稱不算訊號。完整規則見 [MECHANISM.md](MECHANISM.md)。
